...
declare module "./ArrayUtils" {
  ...
  interface IchangePosition_v1 {
    <T>(array: T[], old: number, new_pos: number): T[] | string;
  }

  interface IcheckArrayElementsConstructor_v1 {
  ...
  namespace ArrayUtils {
    namespace FilterCallbacks {
      ...
      export interface startsWithString extends filterCallbackMaker<string> {}
...
const makeValidation = Conditioner.makeValidations;
...
export const filterArray: filterArray = (arr, filterCb) => arr.filter(filterCb);

export const grabPartToEdit: grabber = (getStart, getEnd) => content =>
  [
    content,
    [...content].splice(getStart(content), getEnd(content, getStart(content))),
...
export namespace FilterCallbacks {
  ...
  export const filterConditionIndexTuple: filterConditionIndexTuple = e => e[0];
  ...
  export const definerMaker: definerMaker =
    (toBeIncludedInLine, includerArg, replaceArgs, includer = includesString) =>
    line => {
  ...
  export const declarationAdderMaker: declarationAdderMaker =
    (declarations, customTagMaker, typeDefiner = FilterCallbacks.typeDefiner) =>
    details => {
...
// flip the names
const e = ensureArray;
const mcb = MapCallbacks;
const mv = makeValidation;
