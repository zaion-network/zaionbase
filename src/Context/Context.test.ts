...
import { NonNullableMap } from "../using DeeperUtilities/using DeepUtilites/using LowUtilities/using Utilities/using HighUtilities/JavaScript/MapUtils";
...
describe("Context", () => {
  it("dovrebbe cambaire correttamente il contesto", () => {
    ...
    expect(newctx1.value).toBe("connected");
    const newctx2 = new NotConnected(ctx).boogalor().ctx;
    expect(newctx2.value).toBe("notconnected");
  });

  it("dovrebbe funzionare con i mixin", () => {
    ...
    expect(ctx.changeValue).toBeTruthy();
  });
  ...
  it("da finire", () => {
    ...
    class NumberMap<V> extends NonNullableMap<number, V> {}
    ...
    const res = cb2(state2A);
    expect(res).toEqual(1100);
  });

  it("dovrebbe tornare false", () => {
  ...
  it("da finire", () => {
    ...
    expect(opmap.get(condition)(state)).toEqual("connected");
  });
